...
当官方净值、持仓或可实时估值资产行情缺失时，基金会被跳过，并在任务摘要中记录原因。估值阶段不直接请求外部数据源，只读取 `market_quotes` 中已保存的行情；指数法估算如果发现本地指数行情缺失或 `trade_date` 早于估算日期，会记录 `missing_index_quote` 或 `stale_index_quote`，并按策略规则尝试回退到持仓加权法。开放式基金估算前还会校验最新官方净值日期，若早于当前预期净值日期，则跳过估算并返回 `stale_nav`，避免用历史净值作为当日估算基准。
...
指数行情渠道顺序不在代码中硬编码，由 `index_quote_source_status` 表配置和统计。系统默认初始化东财实时、新浪实时、腾讯实时渠道；每次调用后按“是否为当前缺失指数返回行情”记录成功或失败。后续调用按默认优先级、历史成功率和连续失败次数计算动态顺序；连续失败达到阈值后进入临时冷却，长期连续失败后自动禁用该渠道，避免低成功率渠道长期阻塞实时估值。历史日 K 不作为净值估算的指数行情兜底，实时渠道都失败或未命中时，本轮指数行情视为缺失。
