...
指数行情渠道顺序不在代码中硬编码，由 `index_quote_source_status` 表配置和统计。系统默认初始化东财 HTTP 实时、东财 AkShare 实时、新浪 AkShare 实时、新浪 HTTP 实时、腾讯 HTTP 实时、雪球 HTTP 实时渠道；东财 HTTP 渠道通过 `push2.eastmoney.com/api/qt/ulist.np/get` 获取行情，中证 `93xxxx` 主题指数使用 `secid = 2.<index_code>`。每次调用后按“是否为当前缺失指数返回行情”记录成功或失败。后续调用按默认优先级、历史成功率和连续失败次数计算动态顺序；连续失败达到阈值后进入临时冷却，长期连续失败后自动禁用该渠道，避免低成功率渠道长期阻塞实时估值。历史日 K 不作为净值估算的指数行情兜底，实时渠道都失败或未命中时，本轮指数行情视为缺失。
