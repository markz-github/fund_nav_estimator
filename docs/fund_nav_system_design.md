...
| `data_sources/composites/` | 组合多个渠道的数据源编排器，例如指数行情按实时渠道成功率动态排序 |
...
| 方法 | 语义 | 典型用途 |
|---|---|---|
| `is_exchange_traded_fund(fund)` | 场内 ETF 基金 | ETF 自身行情估算 |
...
| 场内 ETF | `FundClassifier.is_exchange_traded_fund()` | 使用 ETF 行情作为自身行情；可刷新指数映射用于展示或后续扩展 | ETF 行情法：使用已入库 ETF 行情，缺失时跳过 | 不加入“缺少跟踪指数”待维护列表；官方净值可用 ETF 行情口径补充 |
...
当官方净值、持仓或可实时估值资产行情缺失时，基金会被跳过，并在任务摘要中记录原因。估值阶段不直接请求外部数据源，只读取 `market_quotes` 中已保存的行情；指数法估算只使用估值日当日的指数行情，若当日指数行情缺失或无涨跌幅，记录 `missing_index_quote`，并按策略规则尝试回退到持仓加权法。开放式基金估算前还会校验最新官方净值日期，若早于当前预期净值日期，则跳过估算并返回 `stale_nav`，避免用历史净值作为当日估算基准。

官方净值刷新后还会通过独立定时质量检测兜底。检测任务按运行时间推导预期官方净值日期：交易日 20:00 前以前一工作日为预期日期，20:00 后以当天为预期日期，周末以前一工作日为预期日期。启用基金若没有净值，或最新 `fund_navs.nav_date` 早于预期日期，会写入 `data_fetch_errors` 并在任务日志中汇总，避免数据源拉取失败后长期沿用历史净值。该任务还会扫描当天 `fund_task_detail_logs`：任一估算策略尝试失败时，记录 `estimate_strategy_failure` 异常，保留失败步骤以及最终策略和状态；同一基金同一天只写入一条该类异常。
...
| `refresh_quote` | 刷新当前已披露持仓资产和指数映射对应的行情，不刷新持仓本身 | 手动任务 |
| `estimate_nav` | 读取已入库行情估算基金当日净值 | 手动任务 |
| `refresh_quote_estimate` | 盘中组合任务：先刷新股票、ETF、指数等行情，再估算净值，不刷新官方净值、持仓或指数映射 | 手动、定时任务 |
| `refresh_index_mapping` | 刷新指数映射 | 单只手动任务 |
| `sync_new_fund_data` | 新增基金后同步数据 | 新增基金流程 |

组合任务作为单个队列任务执行，内部步骤不重新入队。

`refresh_quote` 和 `refresh_quote_estimate` 刷新行情时按固定顺序执行：先刷新股票等非 ETF/非指数资产行情，再刷新 ETF 行情，最后刷新指数行情。`refresh_quote_estimate` 只有在上述行情刷新阶段结束并完成落库后，才进入基金估值阶段；估值阶段不再重复请求 AkShare 或其他外部数据源。

盘中定时任务必须把行情刷新和估算绑定成一个队列任务执行：`refresh_quote_estimate` 默认在交易日 `0,30 9-15` 分触发，内部先刷新当前已披露持仓资产行情和指数行情，再运行估算。`refresh_holding` 仍是低频任务，负责刷新季度级持仓和指数映射；盘中估值任务不能夹带刷新持仓，避免低频数据源拖慢实时估值链路。
...
行情渠道状态由 `index_quote_source_status` 表配置和统计，按 `source_type = index / stock / etf` 分类展示。指数行情渠道顺序不在代码中硬编码，系统默认初始化东财 HTTP 实时、东财 AkShare 实时、新浪 AkShare 实时、新浪 HTTP 实时、腾讯 HTTP 实时、雪球 HTTP 实时渠道；东财 HTTP 渠道通过 `push2.eastmoney.com/api/qt/ulist.np/get` 获取行情，中证 `93xxxx` 主题指数使用 `secid = 2.<index_code>`。每次调用后按“是否为当前缺失指数返回行情”记录成功或失败。后续调用按默认优先级、历史成功率和连续失败次数计算动态顺序；连续失败达到阈值后进入临时冷却，长期连续失败后自动禁用该渠道，避免低成功率渠道长期阻塞实时估值。历史日 K 不作为净值估算的指数行情兜底，实时渠道都失败或未命中时，本轮指数行情视为缺失。股票和 ETF 行情渠道也会记录成功率、连续失败和最近错误，但调用顺序仍遵循当前固定 fallback 流程。
