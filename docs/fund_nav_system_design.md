...
指数行情渠道顺序不在代码中硬编码，由 `index_quote_source_status` 表配置和统计。系统默认初始化东财实时、新浪实时、东财日线、新浪日线、腾讯日线、中证日线、国证日线等渠道；每次调用后按“是否为当前缺失指数返回行情”记录成功或失败。后续调用按默认优先级、历史成功率和连续失败次数计算动态顺序；连续失败达到阈值后进入临时冷却，长期连续失败后自动禁用该渠道，避免低成功率渠道长期阻塞实时估值。
