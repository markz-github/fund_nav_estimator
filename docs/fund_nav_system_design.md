...
场内 ETF 或已有 ETF 净值来源
  -> ETF 行情 / IOPV 法
否则若为指数跟踪基金
  -> 指数法，若本地指数行情缺失或陈旧，先按需补抓一次该指数行情后重试
  -> 指数法非基准类失败时可回退持仓加权法
否则
  -> 持仓加权法
```
...
当官方净值、持仓或可实时估值资产行情缺失时，基金会被跳过，并在任务摘要中记录原因。指数法估算如果发现本地指数行情缺失或 `trade_date` 早于估算日期，会即时补抓一次该指数行情并重新读取；补抓后仍缺失或陈旧时才记录 `missing_index_quote` 或 `stale_index_quote`，并按策略规则尝试回退到持仓加权法。开放式基金估算前还会校验最新官方净值日期，若早于当前预期净值日期，则跳过估算并返回 `stale_nav`，避免用历史净值作为当日估算基准。
