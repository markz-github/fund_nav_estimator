...
自动部署中，后端测试在 runner 上使用缓存的 Python venv 执行。测试依赖通过本地 Nexus PyPI group `http://192.168.50.50:16666/repository/pypi-group/simple/` 安装，并按 `backend/requirements.txt` 与 `backend/requirements-dev.txt` 的 hash 复用或重建 venv；生产业务镜像只依赖项目级基础镜像，不安装 pytest。
