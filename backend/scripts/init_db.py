        ...
        connection.execute(
            text(
                ...
                INSERT INTO fund_latest_snapshots (
                    fund_code, latest_nav_id, latest_estimate_id,
                    target_etf_holding_id, is_deleted
                )
                SELECT
                    f.fund_code,
                    (
                        SELECT n.id FROM fund_navs n
                        WHERE n.fund_code = f.fund_code AND n.is_deleted = 0
                        ORDER BY n.nav_date DESC, n.id DESC LIMIT 1
                    ),
                    (
                        SELECT e.id FROM fund_estimates e
                        WHERE e.fund_code = f.fund_code AND e.is_deleted = 0
                        ORDER BY e.estimate_time DESC, e.id DESC LIMIT 1
                    ),
                    (
                        SELECT h.id FROM fund_holdings h
