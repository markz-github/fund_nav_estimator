...
const dailySources = computed(() => sources.value.filter((item) => item.source_type === 'daily'))
...
async function loadSources() {
  ...
  try {
    ...
    message.value = apiErrorMessage(error, '指数行情渠道加载失败，请确认后端服务。')
  } finally {
    loading.value = false
  }
}
...
<template>
  <main class="page-shell">
    ...
    <section class="detail-hero">
      <div>
        <p class="eyebrow">Market Sources</p>
        <h1>指数行情渠道</h1>
        <p class="subtitle">实时源 {{ realtimeSources.length }} 个，日线源 {{ dailySources.length }} 个。</p>
      </div>
    ...
    <section class="section-title">
      ...
      <span>{{ dailySources.length }} 个</span>
    </section>
    <div class="table-card">
      <table class="responsive-card-table quote-source-table">
        <thead>
        ...
        <tbody>
          <tr v-if="dailySources.length === 0">
            <td colspan="12">暂无日线渠道。</td>
          </tr>
          <tr v-for="(item, index) in dailySources" :key="item.source_key">
            <td data-label="排序">{{ index + 1 }}</td>
            <td data-label="渠道">
              <strong>{{ item.source_name }}</strong>
              <span class="muted code-line">{{ item.source_key }}</span>
